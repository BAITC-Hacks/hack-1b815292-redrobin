...
def evidence_prompt(payload: Any) -> list[dict[str, str]]:
    return _messages(
        ...
        (
            "На входе ровно один черновик. Верни ровно одно решение для его "
            "draft_id, скопировав draft_id без изменений. Проверь достаточность "
            "источников для заявленного change_type. Если реальные before/after "
            "пункты прямо подтверждают сопоставление и изменение, верни supported. "
            "Не требуй внешних источников для редакционной или смысловой разницы "
            "между переданными текстами. Не заменяй детерминированную проверку "
            "сервера и не добавляй новые clause_id. В supporting_clause_ids укажи "
            "только clause_id этого черновика."
        ),
        payload,
    )
